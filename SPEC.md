...
| 질문 | 기본값 (확인 필요) |
|---|---|
| BGM은 어떤 스타일? | 중세 오케스트라 (무료 라이선스 소스 사용) |
| Runway 비용이 너무 높으면? | stock 이미지 + Ken Burns 효과로 fallback |
| 영상 생성 실패 시 유저에게 어떻게 알림? | 인앱 토스트 + 재시도 버튼 |
| NPC 초상화 이미지 소스? | AI 생성 일러스트 3종 (기사/라이벌/현자) |
